...
class DRLGazebo(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.entity_path = os.path.join('/home/nanu/turtlebot_social_navigation', 'src/turtlebot_simulations/turtlebot4_gazebo/models/turtlebot4_sn_world/goal_box/model.sdf')
    ...
    *******************************************************************************"""
    ...
    def generate_random_goal(self):
        ...
        while (((abs(self.prev_x - self.goal_x) + abs(self.prev_y - self.goal_y)) < 4) or (not self.goal_is_valid(self.goal_x, self.goal_y))):
            self.goal_x = random.randrange(-25, 25) / 10
            self.goal_y = random.randrange(-25, 25) / 10
            tries += 1
            if tries > 200:
    ...
    #     # Example of parsing - this will depend on your SDF file's structure
    #     tree = ET.parse('/mnt/data/turtlebot4.sdf')
    #     root = tree.getroot()
    #     # Hypothetical example - look for a tag or attribute indicating a collision
    #     for collision in root.findall('.//collision_detected'):
    #         if collision.text == 'true':
    #             collision_detected = True
    #             break
        
    #     if collision_detected:
    #         self.get_logger().info('Collision detected, resetting simulation...')
    #         self.reset_simulation()
    ...
    def get_obstacle_coordinates(self):
        tree = ET.parse(os.getenv('DRLNAV_BASE_PATH') + '/src/turtlebot_simulations/turtlebot4_gazebo/models/corridor/model.sdf')
        root = tree.getroot()
